function [Foutput,time] = QDS_KOA(P,DA,CA,HA,Td,Pr,Cw,Cr,Cpt,Crt,Ch,Ept,Ert,Eh,C,Q)
tic %开始计时
np = 100; 
Tmax = 2; 
dim = 3;
...
while t < Tmax
    ...
    worstFitness = Order(np); 
    M = M0*(exp(-lambda*(t/Tmax))); 
    for j = 1:np
        R(j) = 0;
        for k = 1:dim
            R(j)=R(j)+(Sun_Pos(n+k)-fit(j,k))^2; 
        end
        R(j)=sqrt(R(j));
    end
    for j = 1:np
        ...
        MS(j) = rand*(sum(Sun_Score)/4-worstFitness)/(Sum); 
        m(j)=(PL_Fit(j)-worstFitness)/(Sum);
    end

    for j = 1:np
        Rnorm(j)=(R(j)-min(R))/(max(R)-min(R));
        MSnorm(j)=(MS(j)-min(MS))/(max(MS)-min(MS)); 
        Mnorm(j)=(m(j)-min(m))/(max(m)-min(m)); 
        Fg(j)=orbital(j)*M*((MSnorm(j)*Mnorm(j))/(Rnorm(j)*Rnorm(j)+eps))+(rand); 
    end

    for j = 1:np
        a1(j)=rand*(T(j)^2*(M*(MS(j)+m(j))/(4*pi*pi)))^(1/3); 
    end
    for j =1:np
        ...
        if rand<rand
            ...
            Positions(j,:)=Positions(j,:).*U1+(Xm+h.*(Xm-Positions(b,:))).*(1-U1); 
        else
            if rand<0.5 %% Eq.(18)
                f=1;
            else
            ...
            L=(M*(MS(j)+m(j))*abs((2/(R(j)+eps))-(1/(a1(j)+eps))))^(0.5); 
            U=rd>rand(1,n);
            if Rnorm(j)<0.5 
                M=(rand.*(1-r)+r);
                l=L*M*U; 
