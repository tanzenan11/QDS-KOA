function Foutput = GSA(P,DA,CA,HA,Td,Pr,Cw,Cr,Cpt,Crt,Ch,Ept,Ert,Eh,time,C)
%% 算法参数
tic   %开始记录时间
[n,A] = size(P);
pc=0.8;pm=0.2;
...
while a_time < time
    ...
    [Temp,~] = size(EC);
    [F,Level] = non_domination_sort(Temp,EC(:,n*2+2:n*2+4));
    EC(:,2*n+1) = Level;
    combine_Chrom = crowding_distance_sort(F,EC,n,EC(:,n*2+2:n*2+4));  %根据适应度值拥挤距离进行选择
    %精英保留产生下一代种群
...
for i =1:np
    ...
    fitvalue=Chrom_son_by(i,n*2+2:n*2+4);
    new_TS=Ts1;
    while new_TS > Tmin
        Ts1=new_TS;
        new_TS=Ts1*Rate;
    ...
    lastpopulation1(i,n*2+2:n*2+4)=fitvalue;
end
end
